...
class IMPORTGIS_OT_georaster(Operator, ImportHelper):
	...
	def execute(self, context):
		...
		#Path
		originalFilePath = self.filepath
		files = []
		directory = ""
		# self.report({'ERROR'}, originalFilePath)
		if os.path.isdir(originalFilePath):
			# import all files in dir
			dirList = os.listdir(originalFilePath)
			files = list(map(lambda d: originalFilePath + d, dirList))
		else:
			# import selected file
			files = [originalFilePath]
